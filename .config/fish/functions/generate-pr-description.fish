function generate-pr-description --argument base_branch
    ...
    set changelog "$(git diff --unified=0 --abbrev=1 --no-prefix $merge_base..HEAD ':(exclude)**yarn.lock' ':(exclude)**pnpm-lock.yaml' ':(exclude)**package-lock.json' ':(exclude)**generated/**')"
